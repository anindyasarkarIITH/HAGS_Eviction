...
def initiate(hyp_params, train_loader, valid_loader, test_loader):
    ...
    '''  
    with open("./np_test_unit_groups.pkl" , 'rb') as i:
        task_dict_test = pickle.load(i)
        
    with open("./np_train_unit_groups.pkl" , 'rb') as j:
        task_dict_train = pickle.load(j)
    '''
...
def train_model(settings, hyp_params, train_loader, valid_loader, test_loader, image_dict, tab_dict, tab_dict_test, task_dict_test, task_dict_train):
    ...
    def evaluate(er, best_sr, epoch, model_pred, model_search, optimizer, criterion, test=False):
        ...
        if (recall> best_sr):
            ...
            df_search_stat['method'] = 'our_vis'
            df_search_stat['budget'] = 25
            df_search_stat['positive_rate'] = 5
            df_search_stat.to_pickle("./pickle/exp_our_vis_25_050.pkl")
            df_search_stat.to_csv("./csv/exp_our_vis_25_050.csv")
    ...
    checkpoint_pred = torch.load("./model_pred_5_vis")
    
    model_pred.load_state_dict(checkpoint_pred['agent'])  
    #uncomment this if we want to start training from a pretrained model
    checkpoint_search = torch.load("./model_search_5_vis")
    
    model_search.load_state_dict(checkpoint_search['agent'])  
